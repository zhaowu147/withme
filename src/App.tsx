...
import type { ChatState, Message, AIGender, UserSettings } from './lib/types';
...
export default function App() {
  ...
  const aiAvatarUrl = getAvatarUrl(settings.aiGender);
  ...
  const handleSettingsUpdate = (updates: Partial<UserSettings>) => {
    setState(updateSettings(state, updates));
  };
  ...
  const chatBgStyle: React.CSSProperties = settings.chatBackground
    ...
    <div className="h-full flex flex-col bg-washi">
      ...
      <header className="relative flex items-center justify-between px-4 py-3 border-b border-sakura/20 bg-gradient-to-r from-washi via-white to-washi">
        <div className="flex items-center gap-3">
          <img src={aiAvatarUrl} alt={aiName} className="w-9 h-9 rounded-full object-cover border border-sakura/30" />
          <div>
          ...
          <>
            ...
            <div className="absolute right-4 top-14 bg-white rounded-xl shadow-lg border border-washi-dark py-2 z-50 min-w-[160px]">
              <button
                ...
                className="w-full text-left px-4 py-2 text-sm text-text-primary hover:bg-washi-dark transition-colors"
              >
                設定
              </button>
              <button
      ...
      {/* Messages */}
      <div className={`flex-1 overflow-y-auto px-4 py-6 relative ${settings.chatBackground ? '' : 'chat-bg'}`} style={chatBgStyle}>
        <div className="max-w-3xl mx-auto">
          {state.messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-center py-20">
              <div className="relative mb-6">
                <img src={aiAvatarUrl} alt={aiName} className="w-24 h-24 rounded-full border-2 border-sakura shadow-lg object-cover" />
          ...
          {state.messages.map((msg) => (
            <ChatMessage
              ...
              userAvatarUrl={settings.userAvatar}
            />
          ))}
          ...
          {loading && (
            <div className="flex justify-start mb-4 message-enter">
              <div className="flex gap-3 max-w-[80%]">
                <img src={aiAvatarUrl} alt={aiName} className="w-9 h-9 rounded-full flex-shrink-0 object-cover" />
      ...
      {/* Settings modal */}
      {showSettings && (
        <Settings settings={settings} onUpdate={handleSettingsUpdate} onClose={() => setShowSettings(false)} />
      )}
    </div>
  );
}
